/* Warm 2.8.0: UI adapter for the single, independently testable planner. */
(()=>{
'use strict';
...
function evaluate(candidate,params=input()){
 ...
 return candidate.engineeringV260={version:'2.8.0',hardOK:hard.ok,hard,score:quality.score,quality:quality.values,proven:hard.ok};
}
...
runEngineWorkerV1=function(params){return new Promise((resolve,reject)=>{
 const worker=new Worker('./engine-worker-v120.js?v=280-worker1');
...
document.querySelector('.eyebrow')?.replaceChildren(document.createTextNode('V2.8 · точная правка'));
document.title='Тёплый пол — V2.8';
const serialize=serializeState;serializeState=function(){const r=serialize();r.versionLabel='2.8.0';r.roomAdded=[...(state.roomAdded||[])];r.roomRemoved=[...(state.roomRemoved||[])];r.shapeStepMm=state.shapeStepMm;
